...
/*
bool Board::collisionCheck(bool tetro_blocks[4][4], int steps, int cur_x)
{
    int left_most = 999;
    int right_most = -999;
    int bottom_most = -999;
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            if (steps + y >= 0 && steps + y < 20 && tetro_blocks[y][x] && (blocks[steps + y][cur_x + x] != -1)) {
                return true;
            }
        }
    }
...
bool Board::topCheck(bool tetro_blocks[4][4], int steps)
{
    int top_most = 999;
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
...
void Board::addBlocks(bool tetro_blocks[4][4], int steps, int cur_x, int color_id)
{
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            if (tetro_blocks[y][x] && steps + y < 20 && cur_x + x < 10 && steps + y >= 0) {
                if (blocks[steps + y][cur_x + x] == empty) {
                    numOfPoints += 4;
                }
    ...
    for (int i = 0; i < 20; ++i) {
        for (int j = 0; j < 10; ++j) {
            if (blocks[i][j] != empty) {
                ...
                assert(0 <= blocks[i][j] && blocks[i][j] < NumOfColors);
